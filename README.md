...
- **On-device AI** — Gemma 4 E2B via LiteRT; zero data leaves the phone
...
| AI model | Gemma 4 E2B (multimodal, on-device, 2.58 GB) |
...
- Java 17 JDK
- Physical ARM Android device (4 GB+ RAM) with USB debugging enabled
...
4. First analysis → Downloads Gemma 4 E2B model (~2.58 GB, one-time, Wi-Fi recommended)
...
│   └── ResultCard.tsx
├── modules/
│   ├── ai/                 # LiteRT + Gemma 4 E2B integration
...
```
📷 Camera → 📝 Symptoms → 🤖 Gemma 4 E2B (on-device) → 📋 RAG Validation → ✅ Result
                                    ↓
                           📚 NHM/WHO Guidelines
...
3. **Infer**: Gemma 4 E2B analyzes photo + symptoms on-device via LiteRT
