...
export default function VoiceScreen() {
   ...
   * - Telugu (Vosk): ✓ if Vosk model downloaded/loaded, ↓ if not
   * - Other Whisper languages: ✓ if Whisper model downloaded/loaded, ↓ if not
   */
    ...
    if (isGoogleLang(langCode)) {
      ...
      if (whisperStatus === 'not_downloaded') {
        Alert.alert(
          ...
          `${VOICE_LANGUAGES.find(l => l.code === langCode)?.label ?? langCode} requires a 181MB Multilingual Whisper model for offline speech.\n\n` +
  ...
  const downloadOfflineModel = async (locale: string) => {
    Keyboard.dismiss();
    ...
    try {
      if (isGoogleLang(locale)) {
        ...
        if (result.status === 'download_success') {
          await markLocaleDownloaded(locale);
    ...
    if (isGoogleLang(selectedLanguage)) {
      ...
      if (!isOnline && !hasOffline) {
        setError(
          ...
          'Please connect to internet, or download the offline model by tapping the language chip again.'
        );
      } else {
      ...
      if (whisperStatus === 'not_downloaded') {
        Alert.alert(
          'Offline Speech Model',
          `You are offline. To use this language offline, you need to download the 181MB Multilingual Whisper model.\n\n` +
      ...
      {whisperStatus === 'not_downloaded' && !isGoogleLang(selectedLanguage) && (
        ...
        >
          <Text style={styles.whisperBannerText}>
            📥 Download offline speech model (~181 MB) for regional languages in airplane mode
          </Text>
        </TouchableOpacity>
      )}
